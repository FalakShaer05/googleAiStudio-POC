...
def generate_image_in_reference_style(
    ...
    try:
        ...
        # IMPORTANT: In the Gemini request below we want:
        # - Image 1 = SOURCE person (content to keep)
        # - Image 2 = REFERENCE style (visual language to copy)
        # So here we keep the variable names intuitive, but when we build the
...
def generate_fifa_worldcup_card(
    ...
    Uses a single Gemini call with images + form inputs prepended above the user prompt.
    """
    from utils.prompts import build_fifa_raw_context
    ...
    try:
        ...
        context = build_fifa_raw_context(
